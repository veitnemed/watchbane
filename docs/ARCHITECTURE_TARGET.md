...
Документ фиксирует **целевую** структуру папок Terminal Movies Learn перед большим
рефакторингом. Это карта направления, а не текущее состояние кода.

> Статус: проектное соглашение. На момент написания файлы НЕ перенесены, импорты
> и поведение программы НЕ изменены. Перенос делается отдельными маленькими шагами.
...
├─ ui/            # консольное меню, input/print, формы, подтверждения, маршрутизация
...
(`common` — самый нижний; `ui` — самый верхний и единственный «оркестратор».)
...
**Отвечает за:** консольное меню, input, print, confirmations, формы, красивый
вывод, маршрутизацию пользовательских действий.

**Будущие файлы:**
- `ui/main_menu.py`
- `ui/candidate_menu.py`
- `ui/dataset_menu.py`
- `ui/model_menu.py`
- `ui/forms.py`
- `ui/printing.py`
...
**Правило:** ui спрашивает пользователя и показывает результат. Работу делают
`dataset` / `candidates` / `model` / `apis`.

**Текущие файлы, которые относятся сюда:**
- `interface/ui.py`
- `interface/request.py`
- `interface/global_menu.py`
- `interface/interface_funcs.py` — **только** UI/оркестрация;
- `interface/candidate_pool_ui.py`
- `interface/tags_menu.py`
- `interface/backup_menu.py`
- `interface/menu_state.py`, `interface/title_presenters.py`.
...
model       → config, common, storage (только чтобы читать weights); НЕ ui
apis        → config, common; НЕ dataset / candidates / ui / model
...
1. `ui` не сохраняет dataset/candidate_pool напрямую.
2. `ui` не вызывает API напрямую.
3. `apis` не пишут в dataset/candidate_pool.
4. `model` не импортирует `ui`.
5. `common` не импортирует dataset/storage/candidates/model/apis/ui.
6. `config` не импортирует dataset/candidates/model/apis/ui/storage.
7. `candidates` не вызывает `input()` / `print()`.
8. `storage` не содержит пользовательские сценарии.
...
На этом шаге **создан только документ** `docs/ARCHITECTURE_TARGET.md`. Файлы не
переносились, импорты и код не менялись, JSON-форматы и меню не трогались, старые
папки не удалялись, compatibility wrappers не создавались. Поведение программы не
изменено.

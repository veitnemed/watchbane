...
1. **Не расширять модель без переобучения** — все внешние жанры маппятся в существующие 9 `has_*` из `config/genre_tags.json`.
...
- Не добавлять новые `has_*` в `genre_tags.json` без задачи на переобучение модели.
...
| Расширение `genre_tags.json` «по ходу» | Явный запрет без переобучения модели |

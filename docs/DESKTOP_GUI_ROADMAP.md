...
| Candidate pool operations | console via `candidates.service`; desktop **Сервис** tab (dedupe, retry KP, clear) | in progress |
...
- pool maintenance (retry KP, dedupe, TMDb build/import) в desktop (см. Этап 5).

## Этап 5. Candidate pool operations

Статус: console done; desktop planned.
...
Desktop planned (write только с confirmation dialogs):
...
- `Analytics`;
- `Сервис` (Settings/Tools) для редких pool-действий;
- расширение **Сервис**: TMDb import/build, poster batch.

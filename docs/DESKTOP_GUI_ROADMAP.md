...
| Information/analytics read-only | `desktop/analytics/view.py`, `desktop/analytics/chart_constructor.py` | done |
...
- watched genre distribution;
- candidate pool genre distribution;
- chart constructor with bar/function modes;
- local dependencies by year, genre, country, TMDb rating/votes/popularity and user score where available;
- fallback без WebEngine.

Правило: analytics читает dataset/meta/candidate pool и ничего не сохраняет.

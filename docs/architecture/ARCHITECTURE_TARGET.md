...
Runtime SQLite DB, WAL/SHM, legacy JSON exports и generated-файлы из
`data/` не коммитятся. Legacy JSON paths живут в
`scripts/migrations/legacy_paths.py` только для import/export.
...
- runtime-фильтр не пересобирает и не портит сохранённый pool;
...
- отчёты для разработки.

Скрипт может импортировать активные слои, но приложение не должно зависеть от скрипта как от runtime-модуля.

Если скрипт становится нужным для обычного пользовательского сценария, его логика переносится в `dataset`, `candidates`, `storage` или `apis`, а в `scripts` остаётся только CLI-обёртка.
...
- если из legacy нужна идея, переносим её в активный слой явно и с тестами.
...
- runtime SQLite DB и generated/legacy compatibility files живут под `data/` и игнорируются git;
...
2. Укрепить границу `ui -> service -> storage`: убрать прямые записи из UI, если они ещё остались.

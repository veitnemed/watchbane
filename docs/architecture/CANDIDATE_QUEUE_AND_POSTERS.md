# Очередь кандидатов и постеры

## Текущие runtime-гарантии (июль 2026)

- Открытие Watchbane не запускает сразу replenish или пересборку сохранённого candidate pool.
- Первый переход от пустой колоды к заполненной onboarding-колоде всегда запускает gate подготовки постеров.
- Видимая колода ждёт до восьми секунд активный batch постеров, показывая детерминированный прогресс; кэшированные batch обычно проходят сразу.
- Разрешение постера предпочитает выбранный data language, затем переиспользует любой валидный кэшированный постер из root-записи или другой локали, прежде чем делать сетевой запрос.
- Локализованный poster URL, которого нет в кэше, больше не скрывает уже кэшированный постер того же тайтла.
- Загрузки постеров кандидатов ограничены четырьмя параллельными запросами и никогда не выполняются на UI-потоке.
- Неудачный CDN-запрос оставляет стабильный placeholder и явный статус unavailable; он не блокирует навигацию и не меняет recommendation ranking.

Эти гарантии покрыты regression-тестами startup, deck reveal, presenter fallback и poster-prefetch.

Этот документ описывает текущий runtime-путь вкладок **Фильтры** и **Кандидаты**: от сохранённого `candidate pool` до видимой очереди рекомендаций, а также отдельный путь подгрузки постеров.

## Короткая схема

1. `candidate pool` хранится локально в SQLite и читается через `candidates.service`.
2. Вкладка **Фильтры** сохраняет направление поиска и применяет локальные фильтры к уже сохранённому пулу.
...
| Зона | Компонент | Роль |
| --- | --- | --- |
| Чтение пула | `candidates.service.get_search_overview_view()` | Возвращает сохранённые кандидаты и статистику пула. |
...
### 1. Загрузка сохранённого пула

Вкладка **Кандидаты** не ходит в TMDb сама по себе. Она строит очередь из уже сохранённого локального `candidate pool`.
...
`CandidateListView._load_pool_for_deck()` берёт кандидатов из текущего `session.overview()` и передаёт их в `RecommendationDeckService` как словарь по `candidate_detail_identity`.
...
- период выхода;
- страны;
- включённые и исключённые жанры;
- годовой диапазон;
...
- не проходит viability-gate для тайтлов без надёжного рейтинга;
...
5. если локально кандидатов всё ещё мало, UI может запросить TMDb-добор через вкладку **Фильтры**.
...
Если подходящих локальных кандидатов меньше 65, создаётся pending replenish intent. Сначала всё равно применяется локальный поиск, чтобы пользователь сразу увидел то, что уже есть. После завершения локального поиска стартует `FilterReplenishWorker`.
...
Например, если выбраны две страны и `media_type=both`, получится до четырёх bucket:
...
TMDb URL нормализуется к preview-size `w342`, чтобы не качать слишком тяжёлые изображения.
...
При открытии конкретного кандидата запускается ещё один путь:
...
### 2. Есть metadata, но локальный файл ещё не скачан
...
UI не ждёт бесконечно. После дедлайна 2.5 секунды список раскрывается, даже если часть сетевых запросов ещё идёт или уже упала.
...
Qt-путь дополнительно проверяет, что байты открываются как `QImage`. Если сервер вернул HTML, пустой ответ или повреждённые данные, файл не считается готовым.
...
- нет прав на каталог;
- каталог удалён во время работы;
- файл занят;
...
### 10. Lazy localized enrichment ещё не успел отработать
...
Логика допускает fallback на другой localized-блок, если он уже есть в записи. Но если другой localized-блок ещё не был загружен через Details, fallback не из чего выбрать.

Практически это выглядит так:

- RU-постера нет;
- EN-постер в TMDb есть;
- но EN localized block ещё не сохранён в candidate payload;
- до Details enrichment UI видит `missing` или только старый `poster_path`.

### 12. Кандидат был заменён или запрос устарел
...
| `metadata_only` | URL/path metadata есть, но локального файла ещё нет. |

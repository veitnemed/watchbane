...
## Roadmap ID — необязательный контекст

- Используй существующий ID из PRODUCT_ROADMAP (`C1-01`, `C2-03`, …), если пользователь его назвал или работа прямо соответствует такой записи.
- Не создавай, не назначай и не ищи новый ID только ради начала или продолжения работы.
- Не расширяй scope («заодно» запрещено): при отсутствии ID границей остаётся явно поставленная пользователем задача.
- Обновляй `PRODUCT_ROADMAP_CONTRACT.md` (чекбокс + журнал), только если фактически изменена отслеживаемая roadmap-задача или фаза.
- Допиши краткую запись в [`cursor-work.md`](cursor-work.md) после осмысленной сессии (что сделали, коммит, next).

---

## Автономное выполнение (без подтверждения шага)

Перед правками продукта агент коротко фиксирует scope в commentary, но **не ждёт** отдельного `ок` / `делай`. Если есть существующий релевантный roadmap ID, его можно указать как контекст; если его нет, агент работает напрямую в пределах явного запроса.

### Scope note (обязательный формат)

```text
Запрос: <кратко своими словами>
Фаза / активный фокус: <из PRODUCT_ROADMAP>
Соответствует фазе C (C0–C4): да / нет
Roadmap context: <существующий ID или «не требуется»>
Риски размытия: <1–3 пункта или «нет»>
Предложение: выполнить / отклонить / сузить scope
Режим: начинаю выполнение автоматически
```

Отдельное подтверждение всё ещё обязательно только для действий, которые трудно отменить или выходят за поставленную задачу: `git commit` / `push`, удаление пользовательских данных, Factory Reset, изменение внешних систем и публикация.

Scope note можно сократить до одной строки, когда пользователь уже назвал ID, запрос — очевидный прямой фикс, либо меняются только docs/README/PRODUCT/AGENTS.

### Классификатор запросов

| Тип | Действие |
| --- | --- |
| Есть ID из PRODUCT (`C1-05`, `C2-01`, …) | Зафиксировать scope note → делать только этот scope |
| Баг happy path (колода / постеры / 3 кнопки / пустой экран) | Исправить напрямую в рамках запроса; существующий ID можно указать, если он явно подходит |
| Новая фича / вектор A / B / V0 «Сегодня» / web / like-dislike / LLM | **Отклонить** + риски + куда в backlog |
| Полировка «заодно» / «ещё и …» | Отклонить или вынести в отдельный явно согласованный scope |
| Рефакторинг «по пути» | Только если **блокирует** текущую задачу; иначе отклонить |

### Шаблон отказа (копировать)

> Запрос выходит за фазу C или размывает поставленный scope.
> Риск: снова размыть продукт в поисковик, настройки или новую идею.  
> Могу: (1) отклонить, (2) записать в backlog A/B, (3) сузить до конкретного scope.
...
2. **Скрины** в `screens/tmp_ui/<task-label>/`. Имена:
...
На Windows скрипты обычно сами ставят `QT_QPA_PLATFORM=windows`. Output → `screens/tmp_ui/<task-label>/`.
...
- либо live `start_app.py` + сохранение скрина в `screens/tmp_ui/<task-label>/`;
...
UI: capture-скрипт → screens/tmp_ui/<task-label>/ → Read PNG (1.0 и 1.25) → happy path A–E
После: обнови docs/contracts/PRODUCT_ROADMAP_CONTRACT.md, только если менялся статус существующей roadmap-задачи
```

Если существующий ID можно безопасно определить из PRODUCT, агент может указать его без отдельного подтверждения. Отсутствие ID не блокирует прямую работу по ясному запросу; направление нужно только когда сам scope материально неоднозначен или расширяет продукт.

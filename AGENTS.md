...
- Допиши краткую запись в [`cursor-work.md`](cursor-work.md) (что сделали, коммит, next).

---

## Автономное выполнение (без подтверждения шага)

Перед правками продукта агент определяет roadmap ID и коротко фиксирует scope в commentary, но **не ждёт** отдельного `ок` / `делай`. Если следующий безопасный шаг очевиден и укладывается в этот ID, агент продолжает сам.
...
Исключения (scope note можно сократить до одной строки ID):

- пользователь уже указал roadmap ID;
- чисто docs/README/PRODUCT/AGENTS без product-кода.

### Классификатор запросов

| Тип | Действие |
| --- | --- |
| Есть ID из PRODUCT (`C1-05`, `C2-01`, …) | Зафиксировать scope note → делать только ID |
| Баг happy path (колода / постеры / 3 кнопки / пустой экран) | Выбрать ближайший ID → выполнить |
...
> Предложу отдельный roadmap ID перед выполнением.
...
Если ID можно безопасно определить из PRODUCT, агент начинает его без отдельного подтверждения. Если выбрать ID нельзя или запрос расширяет продукт, агент останавливается и просит направление.

...
def test_fixture_traces_keep_adult_through_full_details_and_report_merge_loss(tmp_path: Path) -> None:
    ...
    assert true_trace["paths"]["full_details"]["loss_layer"] == "storage_normalization_or_persistence"
    ...
    assert true_trace["paths"]["filter_details_merge"]["loss_layer"] == "normalization_or_detail_merge"
    assert true_trace["paths"]["filter_details_merge"]["eligibility"]["adult_based_blocked"] is False
...
def test_run_audit_isolated_and_evidence_has_no_title_or_tmdb_id(tmp_path: Path) -> None:
    ...
    assert json.loads((output / "summary.json").read_text(encoding="utf-8"))["loss_count"] >= 1

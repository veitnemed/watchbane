...
**Личный inbox рекомендаций** — разобрать колоду кандидатов (смотрел / сохранить / скрыть), а не стриминг и не «выбор на вечер».
...
Watchbane — локальное Windows-приложение: **конечная колода** (фильм или сериал) с постерами для разбора рекомендаций, с учётом watched / saved / hidden и метаданных TMDb. Продуктовый target колоды — **до 10** карточек. Канон: [PRODUCT_ROADMAP_CONTRACT](docs/contracts/PRODUCT_ROADMAP_CONTRACT.md) (**вариант X — inbox**). Карта документации: [docs/README.md](docs/README.md).
...
- **Своя колода.** Конечный набор с постерами вместо бесконечного скролла.
- **Три решения.** **Смотрел** / **сохранить** / **скрыть** (не «выбрал смотреть сегодня»).
- **Фильмы и сериалы вместе.** Одна коллекция.
- **Понятные сигналы.** Жанры, страна, год, рейтинг TMDb.
- **Запас колоды.** Индикатор на вкладке Рекомендации. [Отчёт](internal/archive/docs/reports/2026-07/otchet_deck_reserve_indicator.md).
- **Локальный пул.** Переживает перезапуск.

...
- read-only компактная карточка с FTS explain (до 3 строк причин) и actions (transfer to watched, hide);
...
- pool maintenance (dedupe, TMDb build/import) — desktop Settings (см. Этап 5).

## Этап 5. Candidate pool operations

Статус: **desktop MVP done** (poster batch — Этап 6).
...
- dedupe, TMDb import/build — **done** в Settings → «Управление pool»;
- poster batch — **не в GUI**, только console (Этап 6).

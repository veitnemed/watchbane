"""Считает жанры сериалов из внешнего API по всему локальному датасету."""
...
def extract_genres(movie_json: dict) -> list:
    """Достает список жанров из полного JSON API."""
    genres = []
...
def print_genre_report(result: dict) -> None:
    """Печатает отчет вида жанр: количество сериалов."""
...
def run() -> None:
    ...
    result = count_genres_from_api(data)

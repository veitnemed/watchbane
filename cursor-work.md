...
**Назначение:** живой журнал изменений в репозитории Watchbane, который ведёт агент после каждой осмысленной сессии; roadmap ID используется как необязательная метка, если он уже есть.
...
### 2026-07-26 — Process: optional roadmap context
- **Запрос:** убрать обязательное требование создавать, назначать или искать task ID для выполнения работы.
- **Сделано:** корневой `AGENTS.md` и активные `.cursor`-правила теперь разрешают прямой фикс по ясному запросу; существующий roadmap ID остаётся необязательным контекстом. PRODUCT обновляется только при изменении реального статуса roadmap.
- **Файлы / коммит:** `AGENTS.md`, `.cursor/rules/{product-phase-c,scope-gate}.md`, PRODUCT, `cursor-work.md`; коммит не запрашивался.
- **Проверка:** docs-only: проверен поиск обязательных ID-gate формулировок; исторические roadmap ID сохранены как журнал и плановые метки.
- **Не сделано / next:** продуктовый код, текущая UI-работа и исторические статусы roadmap не менялись.
...
- **Next:** если нужен UX «убрал → подтянул из запаса до 10» — отдельный явно согласованный scope + правка HAPPY_PATH/теста; без запроса не делать.
...
- **Не сделано / next:** не чинить; C3-04 сессии автора; возможные фиксы — только в отдельном явно согласованном scope.
...
- **Не сделано / next:** ещё 2 авторские сессии; при повторном провале не закрывать C3-04 и разбирать причины в отдельном явно согласованном scope.
